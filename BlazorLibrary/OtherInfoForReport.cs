...
namespace BlazorLibrary
{
    public class OtherInfoForReport
    {
        ...
        public List<List<string>> AsoOther(DateTime? startDate, DateTime? endDate, int? countyes = 0, int? countno = 0)
        {
            ...
            var percentIDS_COUNTYES = BlazorLibrary.Helpers.MathFunctions.Percent(countyes ?? 0, allCount);
            var percentIDS_COUNTNO = BlazorLibrary.Helpers.MathFunctions.Percent(countno ?? 0, allCount);
            ...
            List<List<string>> OtherInfo = new()
            {
              new List<string>()  { StartUIRep["IDS_SESSION_B"], startDate?.ToString() ?? string.Empty},
                ...
                new List<string>()  {StartUIRep["IDS_COUNTYES"] ,$"{countyes}/{allCount} ({percentIDS_COUNTYES}%)" },    //Math.Ceiling((double)((countyes ?? 0) / allCount))*100 TASK 108
                new List<string>()  {StartUIRep["IDS_COUNTNO"] , $"{countno}/{allCount} ({percentIDS_COUNTNO}%)" }   //Math.Ceiling((double)((countno ?? 0) / allCount))*100  TASK 108
            };
            return OtherInfo;
        }
