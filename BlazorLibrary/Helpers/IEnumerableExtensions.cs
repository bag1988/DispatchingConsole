...
namespace BlazorLibrary.Helpers
{
    public static class IEnumerableExtensions
    {
        ...
        public static void AddFiltrItemToFiltr(this IMessage? protoModel, FiltrItem item)
        {
            if (protoModel == null)
                return;
            FieldDescriptor? field = protoModel.Descriptor.Fields.InFieldNumberOrder().FirstOrDefault(x => x.PropertyName == item.Key);
            if (field != null)
            {
                if (field.IsRepeated)
                {
                    ...
                    else if (field.MessageType == FiltrDataOnlyRange.Descriptor)
                    {
                        ...
                        if (!f.Any(x => (x.DateStart?.Equals(dateStart.Date.ToUniversalTime().ToTimestamp()) ?? true) && (x.DateEnd?.Equals(dateEnd?.Date.ToUniversalTime().ToTimestamp()) ?? true) && x.Type == item.Operation))
                        {
                            f.Add(new FiltrDataOnlyRange() { DateStart = dateStart.Date.ToUniversalTime().ToTimestamp(), DateEnd = dateEnd?.Date.ToUniversalTime().ToTimestamp(), Type = item.Operation });
                        }
                    }
            ...
            if (field != null)
            {
                if (field.IsRepeated)
                {
                    ...
                    else if (field.MessageType == FiltrDataOnlyRange.Descriptor)
                    {
                        ...
                        if (f?.Any(x => (x.DateStart?.Equals(dateStart.Date.ToUniversalTime().ToTimestamp()) ?? false) && (x.DateEnd?.Equals(dateEnd?.Date.ToUniversalTime().ToTimestamp()) ?? true) && x.Type == item.Operation) ?? false)
                        {
                            var first = f.First(x => (x.DateStart?.Equals(dateStart.Date.ToUniversalTime().ToTimestamp()) ?? false) && (x.DateEnd?.Equals(dateEnd?.Date.ToUniversalTime().ToTimestamp()) ?? true) && x.Type == item.Operation);
                            f.Remove(first);
                        }
                    }
                    else if (field.MessageType == FiltrTimeRange.Descriptor)
                ...
                if (fields != null)
                {
                    foreach (var field in fields)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                var f = (RepeatedField<FiltrDataOnlyRange>)field.Accessor.GetValue(protoModel);
                                response.AddRange(f.Select(x => new FiltrItem(field.PropertyName, new(x.Type == FiltrOperationType.Range ? $"{x.DateStart?.ToDateTime().ToLocalTime().Date.ToString("d")}-{x.DateEnd?.ToDateTime().ToLocalTime().Date.ToString("d")}" : $"{x.DateStart?.ToDateTime().ToLocalTime().Date.ToString("d")}"), x.Type)));
                            }
                            else if (field.MessageType == FiltrTimeRange.Descriptor)
                            {
                                var f = (RepeatedField<FiltrTimeRange>)field.Accessor.GetValue(protoModel);
