        ...
        private async Task GetList()
        {
            ...
            if (MsgId != null)
            {
                ...
                if (result.IsSuccessStatusCode)
                {
                    try
                    {
                        Model = MsgParam.Parser.ParseJson(await result.Content.ReadAsStringAsync());

                        if (!string.IsNullOrEmpty(Model.MsgName))
                            TitleText = Model.MsgName;
                        else
                            TitleText = GsoRep["IDS_REG_MESS_UPDATE"];
