...
namespace BlazorLibrary.Shared.Abonent
{
    partial class ViewSchedule
    {
        ...
        private async Task SetConnType(ChangeEventArgs e)
        {
            ...
            selectItem.ConnType = ConnTypes.Where(x => selectItem.BaseType == (int)BaseLineType.LINE_TYPE_DIAL_UP ? (new int[] { (int)BaseLineType.LINE_TYPE_DIAL_UP, (int)BaseLineType.LINE_TYPE_PHISICAL }).Contains(x.Number) : x.Number == selectItem.BaseType).FirstOrDefault()?.Number ?? 0;
            selectItem.Beeper = 0;
            selectItem.Address = "";
            selectItem.ConnParam = "";
        ...
        private async Task AddShedle()
        {
            if (selectItem != null)
            {
                ...
                if (string.IsNullOrEmpty(selectItem.ConnParam) || !CheckConnectParam.IsValid(selectItem.ConnParam, (BaseLineType)selectItem.ConnType))
                {
                    //Строка с параметром дозвона пуста или введена с ошибкой! Сохранить запись невозможно.
