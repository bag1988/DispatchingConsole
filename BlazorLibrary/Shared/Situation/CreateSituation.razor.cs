        ...
        public async Task Fire_InsertDeleteAbonent(byte[] AbonentItemByte)
        {
            try
            {
                if (SystemId == SubsystemType.SUBSYST_ASO)
                {
                    ...
                    if (string.IsNullOrEmpty(newItem?.AbName))
                    {
                        await GetListABC();
                        StateHasChanged();
                    }
        ...
        public async Task Fire_UpdateAbonent(byte[] AbonentItemByte)
        {
            try
            {
                if (SystemId == SubsystemType.SUBSYST_ASO)
                {
                    ...
                    if (newItem?.IDAb > 0 && newItem.StaffID > 0 && newItem.SStaffID > 0)
                    {
                        await GetListABC();
                        StateHasChanged();
                    }
                ...
                if (OldList != null && OldList.Any())
                {
                    if (SystemId == SubsystemType.SUBSYST_SZS)
                    {
                        ...
                        if (f != null)
                        {
                            ...
                            if (OldList.Where(x => x.DevType != SubsystemType.SUBSYST_PRD).Any(x => x.CmdID != f.CmdID
            ...
            try
            {
                ...
                if (result.IsSuccessStatusCode)
                {
                    ...
                    if (re != null)
                    {
                        ...
                        lock (Folders)
                        {
                            var newData = re.Select(x => new Objects()
                            {
                            ...
                            var forRemove = Folders.Where(x => x.Type == ListType.MEN).Except(newData).ToList();
                            var forAdd = newData.Except(Folders.Where(x => x.Type == ListType.MEN)).ToList();
                            if (forRemove.Count > 0)
                            {
                                Folders.RemoveAll(x => forRemove.Contains(x));
                                SelectFolders?.RemoveAll(x => x.Key.Type == ListType.MEN && forRemove.Any(r => r.OBJID.Equals(x.Key.OBJID)));
                            }
                            Folders.AddRange(forAdd);
                        }
                    }
                }
            }
            ...
            try
            {
                ...
                if (result.IsSuccessStatusCode)
                {
                    ...
                    if (re != null)
                    {
                        ...
                        lock (Folders)
                        {
                            var forRemove = Folders.Where(x => x.Type == ListType.MAN).Except(newData).ToList();
                            var forAdd = newData.Except(Folders.Where(x => x.Type == ListType.MAN)).ToList();
                            if (forRemove.Count > 0)
                            {
                                Folders.RemoveAll(x => forRemove.Contains(x));
                                SelectFolders?.RemoveAll(x => x.Key.Type == ListType.MAN && forRemove.Any(r => r.Name == x.Key.Name));
                            }
                            Folders.AddRange(forAdd);
                        }
                    }
