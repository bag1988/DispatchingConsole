        ...
        private async Task GetPhonogramListBySess()
        {
            if (SelectSession?.ObjID?.ObjID > 0)
            {
                ...
                if (result.IsSuccessStatusCode)
                {
                    ...
                    var appId = await _localStorage.GetAppIdAsync();

                    foreach (var filePath in arrayFilePath)
                    {
                        await JSRuntime.InvokeVoidAsync("triggerFileDownload", Path.GetFileNameWithoutExtension(filePath), $"api/v1/GetFileByPhonogram?filePath={filePath}&{nameof(CookieName.AppId)}={appId}");
                    }
                }
            }
        }
