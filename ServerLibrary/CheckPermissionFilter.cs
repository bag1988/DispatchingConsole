...
namespace ServerLibrary
{
    public class CheckPermissionFilter : IAuthorizationFilter
    {
        ...
        readonly bool? IsExensions;

        public CheckPermissionFilter(int[] BitsPos, int subSysteID = 0, bool isExensions = false)
        {
            _BitsPos = BitsPos;
            SubSysteID = subSysteID == 0 ? null : SubSysteID;
            IsExensions = isExensions;
        }
        ...
        public void OnAuthorization(AuthorizationFilterContext context)
        {
            ...
            if (IsExensions == true)
            {
                origin = context.HttpContext.Request.Headers.Origin.FirstOrDefault();
                if (!origin?.Contains("chrome-extension") ?? true)
                {
                    origin = null;
                }
            }
    ...
    {
        public CheckPermissionAttribute(int[] BitsPos, int SubsystemID = 0, bool isExensions = false) : base(typeof(CheckPermissionFilter))
        {
            Arguments = new object[] { BitsPos, SubsystemID, isExensions };
        }
    }
}
