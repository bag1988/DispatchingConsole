...
public static class FiltrExtenssion
{
    ...
    public static BinaryExpression? CreateExpressionFromRepeatedDataOnly(this RepeatedField<FiltrDataOnlyRange> values, Expression field, BinaryExpression? filter)
    {
    ...
    static BinaryExpression? CreateExpression<TData>(this RepeatedField<TData> values, Expression field, BinaryExpression? filter) where TData : IMessage
    {
        ...
        foreach (var item in values)
        {
            ...
            else if (item is FiltrDataOnlyRange)
            {
                ...
                filtrType = member.Type;
                fieldType = member.DateStart.GetType();
                constantExpression = Expression.Constant(member.DateStart.ToDateTime().Date.ToTimestamp(), fieldType);
                if (member.DateEnd != null)
                    constantExpressionMax = Expression.Constant(member.DateEnd.ToDateTime().Date.ToTimestamp(), fieldType);
            }
